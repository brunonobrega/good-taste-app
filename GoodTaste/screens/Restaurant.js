...
const Restaurant = ({route, navigation}) => {
  ...
  function editOrder(action, menuId, price) {
    ...
    // Check if the user is adding or removing items
    if(action == '+') {
      // if item alreary exists in cart this just add +1
      if(item.length > 0) {
        let newQuantity = item[0].quantity + 1;
        item[0].quantity = newQuantity;
        item[0].total = item[0].quantity * price;
      } else {
        // if item not exists in cart them add one new item
        const newItem = {
          menuId: menuId,
          quantity: 1,
          price: price,
  ...
  function sumTotalOrder() {
    let totalValue = orderItems.reduce((a, b) => a + (b.total || 0), 0)
    ...
    return (
      <View>
        ...
        >
          ...
          >
            <Text style={{...FONTS.h3}}>{getBasketItemCount()} {verificaItens()} na cesta</Text>
            <Text style={{...FONTS.h3}}>${sumTotalOrder()}</Text>
          </View>
          ...
          >
            ...
            >
              <Text style={{color: COLORS.white, ...FONTS.h2}}>Pedir</Text>
            </TouchableOpacity>
          </View>
